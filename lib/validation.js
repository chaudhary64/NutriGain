...
import { SEXES, ACTIVITY_LEVELS, GOALS } from './tdee';

const ACTIVITY_LEVEL_VALUES = ACTIVITY_LEVELS.map((l) => l.value);
...
export const MUSCLE_GROUPS = ['Abs', 'Back', 'Bicep', 'Chest', 'Forearms', 'Legs', 'Shoulders', 'Tricep'];
...
/**
 * Validate an onboarding payload.
 * `action: 'skip'` marks onboarding done without applying anything.
 * `action: 'apply'` requires the full profile; goals default to the TDEE
 * suggestion when omitted (the server recomputes — client math is never trusted).
 * Returns [errors, normalized] where normalized is
 *   { action, profile, currentWeightKg, targetWeight, macroGoals|null }
 */
export function validateOnboarding(body) {
  ...
  if (action === 'skip') {
    return [null, { action, profile: null, currentWeightKg: null, targetWeight: null, macroGoals: null }];
  }

  const profile = {
    sex: body?.sex,
    age: toFiniteNumber(body?.age),
  ...
  if (!ACTIVITY_LEVEL_VALUES.includes(profile.activityLevel)) {
    errors.push(`activityLevel must be one of: ${ACTIVITY_LEVEL_VALUES.join(', ')}`);
  }
  if (!GOAL_VALUES.includes(profile.goal)) {
    errors.push(`goal must be one of: ${GOAL_VALUES.join(', ')}`);
  }

  let currentWeightKg = null;
  if (body?.currentWeightKg !== undefined && body?.currentWeightKg !== '') {
    currentWeightKg = toFiniteNumber(body.currentWeightKg);
    if (currentWeightKg === null || currentWeightKg < 20 || currentWeightKg > 400) {
      errors.push('currentWeightKg must be a number between 20 and 400 (kg)');
    }
  }

  let targetWeight = null;
  if (body?.targetWeight !== undefined && body?.targetWeight !== '') {
    targetWeight = toFiniteNumber(body.targetWeight);
    if (targetWeight === null || targetWeight < 20 || targetWeight > 400) {
      errors.push('targetWeight must be a number between 20 and 400 (kg)');
    }
  }
  ...
  let macroGoals = null;
  if (body?.macroGoals !== undefined && body?.macroGoals !== null) {
    const [goalErrors, validatedGoals] = validateMacroGoals(body.macroGoals);
    if (goalErrors.length > 0) {
      errors.push(...goalErrors);
    } else {
      macroGoals = validatedGoals;
    }
  }

  if (errors.length > 0) return [errors, null];
  return [null, { action, profile, currentWeightKg, targetWeight, macroGoals }];
}
